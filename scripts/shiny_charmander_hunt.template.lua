...
-- US FireRed party slot 0 @ 0x02024284 (PID +0, OTID +4).
...
local function appendPid(pid, otid, xor_value, shiny)
  local f = io.open(PID_LOG, "a")
  if f then
    f:write(string.format("cycle=%d pid=%08X otid=%08X xor=%s shiny=%s\n",
      cycle, pid or 0, otid or 0, tostring(xor_value), tostring(shiny)))
    f:close()
  end
end
...
local function softReset()
  ...
  for _ = 1, 70 do emu:runFrame() end
  emu:setKeys(0)
  waitFrames(120)
end
...
local function isShinyParty0()
  ...
  if pid == 0 and otid == 0 then return false, pid, otid, nil, true end
  local tid = otid % 65536
  local sid = math.floor(otid / 65536) % 65536
  local p_low = pid % 65536
  local p_high = math.floor(pid / 65536) % 65536
  local xor_value = tid ~ sid ~ p_low ~ p_high
  return xor_value < 8, pid, otid, xor_value, false
end
...
local function saveGame()
  ...
  log("Battery save done")
end

local function onShiny(pid, otid, xor_value)
  local f = io.open(FOUND_PATH, "w")
  if f then
    f:write("shiny=Charmander\nnotify=1\n")
    f:write(string.format("inst=%d\ncycle=%d\npid=%08X\notid=%08X\nxor=%d\nsaved=pending\n", INST, cycle, pid, otid, xor_value))
    f:close()
  end
  ...
  log("SHINY CHARMANDER FOUND + SAVED; wrote STOP_ALL")
end
...
local function oneCycle()
  ...
  takeCharmander()
  local shiny, pid, otid, xor_value, empty = isShinyParty0()
  if empty then
    empty_streak = empty_streak + 1
    log(string.format("EMPTY PARTY streak=%d", empty_streak))
    appendPid(0, 0, -1, "empty")
  ...
  empty_streak = 0
  pushRecent(pid)
  appendPid(pid, otid, xor_value, shiny)
  log(string.format("Charmander pid=%08X xor=%d shiny=%s", pid, xor_value, tostring(shiny)))
  if shiny then
    onShiny(pid, otid, xor_value)
    return true
  end
  return false
end
